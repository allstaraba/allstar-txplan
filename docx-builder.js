...
function buildMarkdownTable(lines) {
  ...
  const firstRowBold = rows[0].every(c => {
    const t = c.trim();
    return (t.startsWith('**') && t.endsWith('**')) || t === '';
  });

  // For 2-col tables, use reference column widths unless first row is bold
...
//
function buildGoalTable(goalNum, goalLines) {
  const rows = [];
...
function parseMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    if (numGoalMatch) {
      ...
      out.push(...buildGoalTable(goalNum, goalLines));
      continue;
    }
